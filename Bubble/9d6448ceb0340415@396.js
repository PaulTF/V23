...
{
  const groups = {
    "inactive": { x: width/5, y: 3*height/6, color: "#808080", cnt: 0, fullname: "Not arrived"},
    "wait": { x: 2*width/5, y: 3*height/6, color: "#FFA500", cnt: 0, fullname: "Waiting"},
    "vac": { x: 3*width/5, y: 3*height/6, color: "#93D1BA", cnt: 0, fullname: "Vaccinating" },
    "obs": { x: 4*width/5, y: 3*height/6, color: "#a579ce", cnt: 0, fullname: "Observation" },
    "finish": { x: 5*width/5, y: 3*height/6, color: "#008000", cnt: 0, fullname: "Finished" },
  };
  return groups
}
);
  ...
  main.variable(observer("chart")).define("chart", ["d3","width","height","nodes","groups","forceCluster","forceCollide"], function(d3,width,height,nodes,groups,forceCluster,forceCollide)
{
  ...
  svg.selectAll('.grpcnt')
    ...
    .text(d => d3.format(",.0f")(groups[d].cnt*7046/1000));
  ...
  let startdate = new Date(2020, 6, 1,7,57)
  ...
  function timer() {
    ...
    svg.selectAll('.grpcnt').text(d => d3.format(",.0f")(groups[d].cnt));
      ...
      circle
     .filter(function(d,i){ return Math.min(d.exit+8,750) == time_so_far; })
     .transition()
     .duration(750)
     .ease(d3.easeCubicIn)
     .attr("r", 0)
    ...
    if (currdate < new Date(2020, 6, 1,20,30)) {
